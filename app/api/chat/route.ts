...
export async function POST(req:Request){ const session=await auth.api.getSession({headers:await headers()}); if(!session)return new Response('unauthorized',{status:401});
 ...
 const convoQ=await db.query(`select c.*, row_to_json(ch.*) as "character" from conversation c join character ch on ch.id=c.character_id where c.id=$1 and c.user_id=$2`,[conversationId,session.user.id]); if(!convoQ.rows[0])return new Response('not found',{status:404}); const convo=convoQ.rows[0]; const character=convo.character as Character;
